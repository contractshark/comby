...
module Make (Language : Language.S) (Metasyntax : Metasyntax.S) = struct
  ...
  let regex_body separator suffix () =
    ...
    let regex_identifier () =
      identifier () >>= fun v -> char separator >> many1 expr >>= fun e -> return (Format.sprintf "%s%c%s" v separator (String.concat e))
    in
  ...
  let reserved_delimiters () =
    ...
    let reserved_comments =
      ...
      |> List.map ~f:string
    in
    [ (reserved_holes () |> List.map ~f:(fun p -> p >>= fun _ -> return ""))
    ; reserved_delimiters
